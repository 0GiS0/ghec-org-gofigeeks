...
# This file contains all file resources specific to generic System and Domain templates

# .gitignore file for System and Domain templates
...
# README file for generic templates
resource "github_repository_file" "generic_template_readme" {
  for_each = {
    for key, value in var.template_repositories : key => value
    if contains(["backstage-template-system", "backstage-template-domain"], key)
  }
  ...
  content             = file("${path.module}/templates/common/docs/README.md.tpl")
  commit_message      = "Add generic skeleton README"
  commit_author       = "Terraform"
  commit_email        = "terraform@${var.github_organization}.com"
  overwrite_on_create = true
...
# Catalog info for generic templates
resource "github_repository_file" "generic_template_catalog_info" {
  for_each = {
    for key, value in var.template_repositories : key => value
    if contains(["backstage-template-system", "backstage-template-domain"], key)
  }
  ...
  content             = file("${path.module}/templates/common/catalog-info.yaml.tpl")
  commit_message      = "Add generic skeleton catalog-info.yaml"
  commit_author       = "Terraform"
  commit_email        = "terraform@${var.github_organization}.com"
  overwrite_on_create = true
